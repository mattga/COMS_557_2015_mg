...
void main(void)
{
	...
	vec4 tex_color_new =  texture(texture_new, pass_TexCoord);
	...
	if(texture_blend == 0)
	{
		color = 0.1 * pass_Color + tex_color + tex_color_light + tex_color_new;
	}
	else if(texture_blend == 1)
	{
		color = tex_color * tex_color_light * tex_color_light;
	}
